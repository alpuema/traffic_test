...
# ============================================================================
# 🎯 ACO OBJECTIVE FUNCTION EXPLAINED
# ============================================================================
"""
WHAT IS THE ACO OPTIMIZING? Understanding the Objective Function

The Ant Colony Optimization algorithm optimizes traffic light timing by minimizing 
a cost function that considers two key factors:

📊 COST FUNCTION BREAKDOWN:
Cost = Average Travel Time + Stop Penalty

1️⃣ AVERAGE TRAVEL TIME (Primary Factor)
   • Definition: Total time spent by all vehicles ÷ Number of vehicles
   • Units: Seconds per vehicle
   • Goal: Minimize how long vehicles spend traveling
   • Impact: ~70-90% of the total cost in most scenarios

2️⃣ STOP PENALTY (Secondary Factor) 
   • Definition: ALPHA × Maximum stop time of any single vehicle
   • Units: Penalty points (ALPHA = 25.0 by default)
   • Goal: Prevent any vehicle from being stuck for too long
   • Impact: ~10-30% of total cost, prevents extreme outliers

🧮 MATHEMATICAL FORMULA:
   Cost = (Total_Travel_Time / Number_of_Vehicles) + (ALPHA × Max_Stop_Time)

🎯 WHY THIS COMBINATION?
   • Average travel time ensures overall efficiency
   • Stop penalty prevents individual vehicles from waiting excessively
   • Balance prevents solutions that sacrifice individual fairness for group efficiency

⚡ WHERE IS THIS IMPLEMENTED?
   • File: src/optimization/simple_aco.py
   • Function: calculate_cost(metrics)
   • Lines: ~452-465
   • Called by: Each ant during solution evaluation

🔧 TUNING PARAMETERS:
   • ALPHA (penalty weight): Higher = more emphasis on preventing long stops
   • Default ALPHA = 25.0 provides good balance for most scenarios
   • You can modify this in simple_aco.py for different optimization priorities

💡 OPTIMIZATION STRATEGY:
   • ACO ants explore different traffic light timings (15-100s green, 3-6s yellow)
   • Each timing combination is evaluated with this cost function
   • Pheromones reinforce successful (low-cost) timing combinations
   • Over iterations, the algorithm converges to optimal signal coordination
"""
...
def get_user_configuration():
    ...
    print(f"\n📈 Show Optimization Progress Plots:")
    print(f"   • Yes: View convergence graphs and results visualization")
    print(f"   • No: Skip plotting (faster for batch runs)")
    
    show_plots_input = input(f"\nShow optimization plots? (y/n) [default: y]: ").strip().lower()
    show_plots = show_plots_input != 'n'
    print(f"✅ Selected: {'Yes' if show_plots else 'No'} - plots will {'be shown' if show_plots else 'be skipped'}")
    
    print(f"\n🖥️  Launch SUMO-GUI (Visual Traffic Simulation):")
    print(f"   • Yes: Watch traffic animation with optimized signals")
    print(f"   • No: Run simulation in background (faster)")
    print(f"   💡 GUI is great for understanding results but slows down execution")
    
    launch_gui_input = input(f"\nLaunch SUMO-GUI with results? (y/n) [default: n]: ").strip().lower()
    launch_gui = launch_gui_input == 'y'
    print(f"✅ Selected: {'Yes' if launch_gui else 'No'} - {'GUI will launch' if launch_gui else 'background simulation'}")
    
    print(f"\n📝 Verbose Output:")
    print(f"   • Yes: Show detailed progress and debugging info")
    print(f"   • No: Show only essential information")
    
    verbose_input = input(f"\nShow detailed progress? (y/n) [default: y]: ").strip().lower()
    verbose = verbose_input != 'n'
    print(f"✅ Selected: {'Yes' if verbose else 'No'} - {'detailed output' if verbose else 'minimal output'}")
    ...
    print("=" * 50)
    print(f"🏗️  Grid: {grid_size}x{grid_size} ({grid_size**2} intersections)")
    print(f"🚗 Vehicles: {n_vehicles}")
    print(f"⏱️  Duration: {sim_time}s ({sim_time/60:.1f} min)")
    print(f"🚦 Pattern: {traffic_pattern.title()}")
    print(f"🐜 Ants: {n_ants}")
    print(f"🔄 Iterations: {n_iterations}")
    print(f"📊 Options: {'plots' if show_plots else 'no-plots'}, {'GUI' if launch_gui else 'no-GUI'}, {'verbose' if verbose else 'quiet'}")
    
    # Estimate runtime
    estimated_time = (n_ants * n_iterations * sim_time) / 60 / 100  # Rough estimate
    print(f"⏱️  Estimated runtime: ~{estimated_time:.1f} minutes")
    print()
