...
def create_mock_grid_categories(grid_size):
    ...
    for x in range(grid_size):
        for y in range(grid_size):
            ...
            if x < grid_size // 2:
                categories['residential'].append(coord)
            else:
                categories['commercial'].append(coord)
                
            if y == grid_size // 2:  # Middle row
                categories['main_roads'].append(coord)
                categories['industrial'].append(coord)
...
def select_mock_coordinate(categories, weights, grid_size):
    ...
    for category, weight in weights.items():
        if category in categories:
            ...
            for coord in coords:
                weighted_coords.extend([coord] * int(weight * 10))
