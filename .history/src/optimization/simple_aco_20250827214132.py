...
def create_optimization_plot(best_costs, best_metrics_history, paths):
...
def generate_ant_solution(n_phases, phase_types, pheromone_weights=None):
    ...
    for i in range(n_phases):
        ...
        # Random exploration vs pheromone guidance
        if random.random() < EXPLORATION_RATE:
            ...
            if pheromone_weights is not None and i < len(pheromone_weights):
                ...
                if influence > 1.5:
                    # High pheromone - bias toward center of range
                    center = (min_dur + max_dur) // 2
                    spread = max(5, (max_dur - min_dur) // 4)  # At least 5s spread
                    biased_min = max(min_dur, center - spread)
                    biased_max = min(max_dur, center + spread)
                    duration = random.randint(biased_min, biased_max)
                else:
                    # Low pheromone - use full range
                    duration = random.randint(min_dur, max_dur)
            else:
                # No pheromone info - random
                duration = random.randint(min_dur, max_dur)
...
def update_pheromones(pheromone_weights, solutions, scores):
    """
    Update pheromone weights based on ant performance.
    ...
    worst_score = max(valid_scores) if len(valid_scores) > 1 else best_score
    
    # Reward good solutions
    for solution, score in valid_pairs:
        # Calculate reward based on relative performance
        if worst_score > best_score:
            reward = 0.2 + 0.8 * (worst_score - score) / (worst_score - best_score)
        else:
            reward = 1.0 if score == best_score else 0.5
        
        # Apply reward to each phase
        for i, duration in enumerate(solution):
            if i < len(pheromone_weights):
                pheromone_weights[i] += reward
    
    # Ensure minimum pheromone level
    pheromone_weights = np.maximum(pheromone_weights, 0.1)
...
def run_simplified_aco_optimization(config=None):
    """
    ...
    try:
        ...
        if SHOW_PLOTS and len(best_costs) > 0:
            create_optimization_plot(best_costs, best_metrics_history, paths)
