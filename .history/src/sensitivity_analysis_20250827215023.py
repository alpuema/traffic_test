...
def run_sensitivity_analysis(
    ...
    if parallel and len(param_combinations) > 1:
        ...
        results = _run_sequential_analysis(
            param_combinations, base_config, n_replications, output_dir
        )
    ...
    plot_files = _generate_sensitivity_plots(results, parameter_ranges, output_dir)
...
def _run_sequential_analysis(param_combinations, base_config, n_replications, output_dir):
    ...
    for i, params in enumerate(param_combinations):
        ...
        for rep in range(n_replications):
            ...
            try:
                result = _run_single_optimization(config)
...
def _run_parallel_analysis(param_combinations, base_config, n_replications, output_dir, max_workers):
...
def _run_single_optimization(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Run optimization with plots disabled for sensitivity analysis
    optimization_result = run_simplified_aco_optimization(clean_config, show_plots_override=False)
